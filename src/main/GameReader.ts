...
export default class GameReader {
	...
	checkProcessDelay = 0;
	isLocalGame = false;
	async loop(dHook: string): Promise<string | null> {
		...
		if (
			...
			if (
				(this.oldGameState === GameState.TASKS || this.oldGameState === GameState.DISCUSSION) &&
				state === GameState.LOBBY
			) {
				console.log(
				...
				const impostorWon = players.filter((player) => player.isImpostor && !player.isDead).length > 0;
				...
				axios({
					...
					data: JSON.stringify({
						content: 'Eine weitere Runde wurde beendet',
						embeds: [
							{
								author: {
									name: `Host: ${players
										.filter((player) => hostId && player.clientId && hostId === player.clientId)
										.map((player) => `${player.name}`)
										.join('')}`,
								},
								title: 'Daten der letzten Runde',
								description: `Diese Runde haben die **${impostorWon ? 'Impostor' : 'Crewmates'}** gewonnen`,
								color: impostorWon ? 16713785 : 569599,
								fields: [
									{
										name: 'Crewmates',
										value: players
											...
											.map(
												(player) =>
													`${player.name} *(${player.isDead ? 'Tod' : 'Lebendig'}${
														player.disconnected ? ' | Verlassen' : ''
													})*`
											)
											.join('\n'),
										inline: true,
									...
									{
										name: 'Sonstige Daten dieser Runde',
										value: `Server: ${this.currentServer}\nMap:${MapType[map]}`,
									},
								],
								thumbnail: {
									url: 'https://img.icons8.com/clouds/500/among-us.png',
								},
								footer: {
									text: `${players.length}/${maxPlayers} Spieler`,
								},
				...
				console.log(
					players.filter((player) => player.isImpostor && !player.isDead).length > 0
						? 'Imposter haben gewonnen'
						: 'CrewMates haben gewonnen'
				);
			}

			if (this.oldGameState !== state) console.log(new Date().toLocaleString(), this.oldGameState, state);
